...
const Page = () => {
  ...
  const getAllIncidentTemplates = async (category, selectedRiskLevel) => {
    console.log("get all incident templates called");
    try {
  ...
  return (
    <div className="flex">
      <div className="mb-2 w-full">
        <div className="border-b border-[#d2e1fb]">
          <Header
        ...
        <div className="sticky top-0 z-40 mb-1 flex justify-between gap-4 bg-[#FBFCFE] p-0 pr-4 sm:py-2 md:flex-row">
          <span></span>
          <div className="flex items-center justify-between gap-2">
            ...
            >
              {filterVisible === "filter" ? (
                <LuFilterX className="size-5 text-lg text-[#002FA7]" />
              ) : (
                <FiFilter className="mr-2 size-5 text-lg text-[#9dbdf4] hover:text-[#002fa7]" />
              )}
            </button>
          </div>
        </div>
